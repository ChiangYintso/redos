...
use crate::memory::{
    addr::*,
    mapping::{mapping::Mapping, page_table_entry::Flags, segment::MapType, segment::Segment},
    range::Range,
    *,
};
use crate::KResult;
...
impl MemorySet {
    /// 创建内核重映射
    pub fn new_kernel() -> KResult<MemorySet> {
    ...
    pub fn from_elf(file: &ElfFile, is_user: bool) -> KResult<MemorySet> {
        // 建立带有内核映射的 MemorySet
    ...
    /// 添加一个 [`Segment`] 的内存映射
    pub fn add_segment(&mut self, segment: Segment, init_data: Option<&[u8]>) -> KResult<()> {
        // 检测 segment 没有重合
    ...
    ///
    /// `segment` 必须已经映射
    pub fn remove_segment(&mut self, segment: &Segment) -> KResult<()> {
        // 找到对应的 segment
